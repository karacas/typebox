...
module.exports = context => {
    ...
    var noCache = context.require('superagent-no-cache');
    ...
    return {
        ...
        init() {
            ...
            this.pushDocSlugRule = (slug, save = false) => {
                return new Promise((resolve, reject) => {
                    request
                        .get(this.config.url_plainDocsBase + slug + '/index.json')
                        .expiration(60 * 60)
                        .then(res => {
            ...
            context.addPermanentRules([
                { title: 'Dev Docs', type: ['DEVDOCroot'], icon: iconDevDocs, params: { changePath: { name: 'Dev Docs', path: 'DEVDOCS_PATH' } } },
                {
                    ...
                    initSort: 15,
                    posFixed: 1,
                    icon: { type: 'iconFont', iconClass: 'feather-toggle palette-Amber-A200 text' },
        ...
        defineTypeExecutors(obj) {
            this.addPlugin = obj => {
                this.config.installedDocs.push(_.get(obj.rule, 'params.obj.slug'));
                this.pushDocRules2Install();
            };
            this.removePlugin = obj => {
                this.config.installedDocs = this.config.installedDocs.filter(e => e !== _.get(obj.rule, 'params.obj.slug'));
                this.pushDocRules2Install();
            };
            this.checkInstalled = obj => {
                return !this.config.installedDocs.includes(_.result(obj, 'params.obj.slug'));
            };
            return [
                {
                    type: 'DEVDOC_SLUG',
        ...
        defineTypeViewers() {
            ...
            viewerComp.internal = context.createViewerHtml(
                ...
                (resolve, reject, rule) => {
                    ...
                    request
                        .get(encodeURI(rule.params.openUrlDoc))
                        .expiration(60 * 60)
                        .then(html => {
                            ...
                            resolve({
                                component: Object(
                                    context.createComponentFromHtml(
                                        '<div id="iFrameView" class="frameInLoad"><Loading></Loading><div class="webview" style="overflow-x: hidden;" id="contentCodeDocs' +
                                            idRule +
                                            '">' +
                                            html.text +
                                            '</div></div>'
                                ...
                                executeJavaScript: () => {
                                    ...
                                    let placeInner = (response, idRule) => {
                                        ...
                                        if (iFrameView) iFrameView.className = iFrameView.className.replace('frameInLoad', 'frameLoaded');
                                    };

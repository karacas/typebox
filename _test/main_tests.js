    ...
    if (true) {
        it("should do change paths", function() {
            ...
            var levels = () => {
                return win
                    .getText("#levelSize")
                    .then(() => win.keys("change1")).then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ENTER_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => win.keys("change2")).then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ENTER_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => win.keys("change3")).then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ENTER_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    ...
                    .then(() => win.keys(C.BACKSPACE_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => win.keys(C.ESCAPE_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => win.keys(C.ESCAPE_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .getHTML('.level_root')
                    .then((val) => assert.equal(val.length > 0, true))
                    .then(() => win.keys(C.ESCAPE_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => this.app.browserWindow.isVisible())
                    .then((val) => assert.equal(val, false)); //ESCAPE OK
            };
        ...
        it("should do history", function() {
            ...
            var history = (type, exp, exp2) => {
                return win
                    ...
                    .then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ENTER_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => win.keys(C.BACKSPACE_KEY)).then(() => Q.delay(C.DELAY_KEY));
            };
    ...
    if (true) {
        it("should do get conext menú & hidden search", function() {
            ...
            var conext = (text, expect1, expect2) => {
                return win
                    .then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(text)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => win.keys(C.CTRL_KEY)).then(() => win.keys(C.ENTER_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => this.app.client.windowByIndex(0).getText(C.RULE_HIGHLIGHTTED_TITLE))
                    .then((val) => assert.equal(val, expect1))
                    .then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.CTRL_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => expect2 ? win.keys(C.DOWN_KEY) : Q.delay(1)).then(() => Q.delay(C.DELAY_KEY)) //KTODO: Revisar el clipboard directo
                    .then(() => this.app.client.windowByIndex(0).getText(C.RULE_HIGHLIGHTTED_TITLE))
                    .then((val) => expect2 ? assert.equal(val, expect2) : Q.delay(1));
            };

            return conext("XXX", "Place Text", "Copy to clipboard") //Test in hidden search
                .then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ESCAPE_KEY))
                .then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ESCAPE_KEY))
    ...
    if (true) {
        it("should do go to root & empty", function() {
            ...
            var goroot = () => {
                return win
                    .getText("#levelSize")
                    .then(() => win.keys("asynctest")).then(() => Q.delay(C.DELAY_KEY)).then(() => win.keys(C.ENTER_KEY)).then(() => Q.delay(C.DELAY_KEY))
                    .then(() => Q.delay(3000))
                    .then(() => win.keys("loa")).then(() => Q.delay(C.DELAY_KEY * 2))
                    .then(() => win.keys("Shift")).then(() => win.keys("Backspace")).then(() => Q.delay(C.DELAY_KEY * 2))
                    .getHTML('.level_root')
                    .then((val) => assert.equal(val.length > 0, true))
            };

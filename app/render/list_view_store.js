'use strict';
const { takeRight, dropRight } = require('lodash');
...
function storeList(state = storeInitialState, action) {
   let obj, newState, text, rulesPathHist, statusBar_actual;
   ...
   switch (action.type) {
      ...
      case 'UPDATE_LIST_FORCE':
         let rulePath = global_aux.cloneDeep(state.rulesPath);
      ...
      case 'CHANGE_RULES_PATH':
         ...
         let sortReverse = objPath.sortReverse;

         let sortByNoRules = objPath.sortByNoRules;
         let sortReverseNoRules = objPath.sortReverseNoRules;
         ...
         if (path !== rootPathInitialState.path) {
            if (path !== takeRight(rulesPathHist)[0].path && !pathObj.avoidHistory) {
               rulesPathHist = rulesPathHist.slice(0, 5);
      ...
      case 'BACK_RULES_PATH':
         rulesPathHist = global_aux.cloneDeep(state.rulesPathHist);

         if (rulesPathHist.length > 1) {
            rulesPathHist = dropRight(rulesPathHist, 1);
         } else {
            rulesPathHist = [rootPathInitialState];
         }

         let rulesPath = takeRight(rulesPathHist)[0];
      ...
      case 'CHANGE_LAST_RULE_SELECTED':
         statusBar_actual = state.filteredRules && state.filteredRules.size ? action.index : 0;
         if (statusBar_actual > 0 && statusBar_actual >= state.filteredRules.size) statusBar_actual = state.filteredRules.size - 1;
         if (statusBar_actual < 0) statusBar_actual = 0;
         return Object.assign({}, state, { lastRuleSelected: action.obj, statusBar_actual });

      case 'CHANGE_LAST_EXECUTORS_SELECTED':
         statusBar_actual = state.filteredRules && state.filteredRules.size ? action.index : 0;
         if (statusBar_actual > 0 && statusBar_actual >= state.filteredRules.size) statusBar_actual = state.filteredRules.size - 1;
         if (statusBar_actual < 0) statusBar_actual = 0;
         return Object.assign({}, state, { lastExcutorSelected: action.obj, statusBar_actual });

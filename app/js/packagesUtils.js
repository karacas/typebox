...
function validatePackage(obj) {
    ...
    let engine = _.result(obj, 'engines.typebox');
    if (!engine) {
        return { validate: false, error: obj.name + ' no engine' };
    }

    let plataforms = _.result(obj, 'plataforms');
...
function checkLocalVersion(name) {
    return _.result(requierePackageJson(name), 'version');
}
...
function checkRemotePackageObj(name) {
    return new Promise((resolve, reject) => {
        ...
        axios
            ...
            .then(res => {
                if (!res.data) {
                    reject(name + ' res.data');
                    return false;
                }
                let version = _.result(res.data, 'dist-tags.latest');
                if (!version) {
                    reject(name + ' no version');
                    return false;
                }
                let obj = res.data.versions[version];
                if (!obj) {
